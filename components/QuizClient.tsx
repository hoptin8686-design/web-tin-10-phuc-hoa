...
export default function QuizClient({
  ...
  const initQuiz = () => {
    ...
    const prepared: PreparedQuestion[] = questions.map((q) => {
      // Xáo trộn phương án cho từng câu
  ...
  const handleNext = () => {
    if (currentIndex < total - 1) {
      ...
      const finalCorrect = picked === currentQ.correctIndex ? correctCount : correctCount;
      ...
      {!isDone ? (
        <div className="mt-6 animate-pop-in">
          ...
          <div className="rounded-2xl border border-star/10 bg-void-card p-5 sm:p-6 shadow-card">
            ...
            {currentQ.code && (
              <pre className="mt-3 overflow-x-auto rounded-xl border border-star/10 bg-[#07131b] p-3.5 font-mono text-xs sm:text-sm text-sea-deep">
                <code>{currentQ.code}</code>
              </pre>
            )}

            {/* Danh sách 4 phương án - Nút bấm TO tối ưu cho mobile */}
              ...
              <div className="mt-5 animate-pop-in rounded-xl border border-star/10 bg-[#07131b] p-4">
                <p className="font-display text-sm font-bold text-star">
                  {picked === currentQ.correctIndex ? (
                    <span className="text-leaf-deep">🎉 Chính xác!</span>
                  ) : (
                    <span className="text-coral">💡 Hãy xem lại kiến thức nhé:</span>
                  )}
                </p>
